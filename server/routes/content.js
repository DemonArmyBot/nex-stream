...
// Configure multer for file uploads (only for video/pdf)
...
router.post('/upload', verifyAdmin, upload.fields([
  ...
  try {
    ...
    if (video) {
      ...
      const isVideo = ['.mp4', '.m3u8', '.mov', '.avi'].includes(videoExt);
      if (isVideo) {
        newContent.push({
          title: req.body.title || video[0].originalname.split('.')[0],
          url: `/uploads/${video[0].filename}`, // Local file URL
          type: 'video',
          category: req.body.category || 'Videos',
          thumbnail: '', // Placeholder; add FFmpeg later
          metadata: { duration: 'Unknown', resolution: 'Unknown', format: videoExt }
        });
      } else {
    ...
    if (pdf) {
      newContent.push({
        ...
        url: `/uploads/${pdf[0].filename}`, // Local file URL
        type: 'pdf',
        category: req.body.category || 'Documents',
        thumbnail: '' // Placeholder; add PDF thumbnail later
      });
    }
    if (text) {
      ...
      lines.forEach(line => {
        ...
        if (match) {
          ...
          const type = url.match(/\.(mp4|m3u8|mov|avi|pdf)$/i) ? 'video' : 'text'; // Detect type from URL
          newContent.push({
            title: match[1],
            url: url, // Use external URL directly
            type: type,
            category: req.body.category || 'Text',
            thumbnail: '', // Placeholder for external content
            metadata: { duration: 'Unknown', resolution: 'Unknown', format: type === 'video' ? 'external' : 'text' }
          });
        }
      });
      if (validLines === 0) {
