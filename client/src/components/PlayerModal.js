...
function PlayerModal({ item, onClose }) {
  ...
  const handleEdit = async (field, value) => {
    try {
      await axios.put(`/api/content/${item._id}`, { [field]: value }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      if (field === 'title') setEditTitle(value);
      if (field === 'thumbnail') setEditThumbnail(value);
    ...
    <div className="player-modal active">
      <div className="player-container">
        ...
        <div ref={viewerRef} className="pdf-viewer" style={{ display: 'none' }}>
          ...
          <div>
            <input type="number" value={newAnnotation.page} onChange={e => setNewAnnotation({ ...newAnnotation, page: parseInt(e.target.value) })} min="1" />
            <input type="text" value={newAnnotation.text} onChange={e => setNewAnnotation({ ...newAnnotation, text: e.target.value })} placeholder="Annotation text" />
            <button onClick={handleAnnotationSubmit}>Add Annotation</button>
          </div>
        </div>
        {isAdmin && (
          <div className="admin-edit">
            <input type="text" value={editTitle} onChange={e => handleEdit('title', e.target.value)} placeholder="Edit Title" />
            <input type="text" value={editThumbnail} onChange={e => handleEdit('thumbnail', e.target.value)} placeholder="Edit Thumbnail URL" />
          </div>
        )}
      </div>
    </div>
