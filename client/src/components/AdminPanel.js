...
function AdminPanel({ refreshLibrary }) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('/api/content/upload', data, {
  ...
  const handleEdit = async () => {
    try {
      await axios.put(`/api/content/${editId}`, { title: editTitle, thumbnail: editThumbnail }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      refreshLibrary();
      setEditId(null);
  ...
  useEffect(() => {
    axios.get('/api/content/library', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
